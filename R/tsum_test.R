...
tsum_test <- function(strscore,
  ...
  assert(
    "trim should be a number that is at least 0 and less than 0.5.", is.numeric(trim),
    trim >= 0, trim < 0.5
  )
  ...
  assert(
    "When specified, cluster should be a cluster object from the parallel package.",
  ...
  if (case_control && strscore$samples[group != "case", .N] < 5) {
    warning(
      "May have too few control samples (n = ", strscore$samples[group != "case", .N], ").\n",
    ...
    if (parallel) {
      ...
      if (is.null(cluster_n)) {
        ...
        if (cluster_n > n_cores) {
          warn.message <- paste0(
            "More threads have been requested by cluster_n (", cluster_n,
  ...
  if (parallel) {
    ...
    strscore_loc_list <- lapply(all_loci, function(loc) {
      strscore[loc]
    })
    tsum_statistic_1locus_failsafe <- function(...) {
      tryCatch(tsum_statistic_1locus(...), error = function(e) {
        NULL
      })
    }
    T_stats_list <- snow::parLapply(
      cl = cluster,
      strscore_loc_list,
  ...
  # Prepare output
  outtsum <- exstra_tsum_new_(strscore,
    tsum = T_stats,
...
tsum_statistic_1locus <- function(strscore_loc,
                                  case_control = FALSE,
                                  min.quant = 0,
                                  trim = 0,
                                  give.pvalue = FALSE,
  ...
  if (give.pvalue) {
    pvs <- p_value_simulation(
      tsums = tsums, qmt_bac_untrim = qmt_bac_untrim,
...
#'
#' @keywords internal
qm_tsum_stat_bare_ <- function(qm,
                               bmu,
                               bstde) {
