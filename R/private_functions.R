...
plot_many_str_score <- function(strscore, typename, plot_cols, loci = NULL,
  ...
  for (i in plot_types) {
    if (i == 2) {
      pdf(paste0(dirbase, typename, ".pdf"), useDingbats = FALSE)
    }
    for (loc in loci) {
      if (i == 3) {
      ...
      plot(strscore,
        loci = loc, sample_col = plot_cols_this,
...
make_quantiles_matrix <- function(strscore, loc = TRUE, sample = NULL, read_count_quant = 1,
  ...
  # remove NA rows
  low.count.samples <- apply(quant.matrix, 1, function(x) {
    all(is.na(x))
  })
