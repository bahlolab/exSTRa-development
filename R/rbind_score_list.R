...
#' score_list <- list(
#'   group1 = exstra_wgs_pcr_2[, "WGSrpt_20"],
...
rbind_score_list <- function(strscore_list, idcol = "data_group",
  ...
  data.new <- rbindlist(lapply(strscore_list, function(x) {
    x$data
  }), idcol = idcol, fill = fill)
  db.new.db <- rbindlist(lapply(strscore_list, function(x) {
    x$db
  }), fill = fill)
  ...
  new_strscore$samples <- rbindlist(lapply(strscore_list, function(x) {
    x$samples
  }), idcol = idcol, fill = TRUE)
  ...
  if (!allow_sample_clash) {
    ...
    if (max(test) > 1) {
      stop(
        "A sample name is duplicated in inputs, for sample names: ",
