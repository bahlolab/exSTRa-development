...
read_exstra_db_ucsc <- function(file, header, ...) {
  ...
  col.classes <- c(
    "integer", "character", "integer", "integer", "factor",
  ...
  if (header) {
    assert(
      "When reading UCSC files with a header, the header lines must be: '#bin' (converted to 'X.bin' by R), 'chrom', 'chromStart', 'chromEnd', 'name', 'period', 'copyNum', 'consensusSize', 'perMatch', 'perIndel', 'score', 'A', 'C', 'G', 'T', 'entropy', 'sequence'",
  ...
  assert(
    "The column classes of the UCSC file are not as expected. Are you sure you are supplying a UCSCS file and that the header setting is correct?",
    prod(vapply(data, class, FUN.VALUE = "") == col.classes) == 1
  )
