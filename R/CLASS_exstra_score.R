...
strs_read_groups_ <- function(counts, groups.regex = NULL, groups.samples = NULL) {
  ...
  if (!is.null(groups.samples)) {
    ...
    assert(
      "groups.samples must be a list if used, with vectors with names of at least one of 'case', 'control' or 'null'.",
    ...
    assert(
      "groups.samples does not currently accept multiple of the same names for groups, please put all sample names in the one vector under that name",
...
plot.exstra_score <- function(x, loci = NULL, sample_col = NULL,
  ...
  for (locus.name in strlocis) {
    ...
    for (xlinked in xlinked_loop) {
      ...
      plot(NA,
        xlim = xlim_1, ylim = ylim, main = main.title,
      ...
      for (samp in c(setdiff(unique(plot_data$sample), names(sample_col)), intersect(unique(plot_data$sample), names(sample_col)))) {
        plot(ecdf(plot_data[sample == samp, rep]),
          add = TRUE, col = replace(sample_col[samp], is.na(sample_col[samp]), black_trans), verticals = verticals,
          pch = pch, ...
        )
      }
      if (plot_data[, .N] == 0) {
